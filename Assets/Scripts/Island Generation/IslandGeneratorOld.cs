...
public class IslandGeneratorOld : MonoBehaviour
{
    ...
    {
        float[,] heightMap = Noise.GenerateWithTexture(txt, padding, xOff, yOff, extHeight, intHeight, extScale, intScale);
    ...
    public void Generate(int area, float xOff, float yOff, float noise, float size)
    {
        ...
        float[,] heightMap = Noise.Generate(area+1, area+1, xOff, yOff, noise);
